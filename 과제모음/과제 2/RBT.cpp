...
public:
    ...
    void erase(T1 key);
    void delete_choose(treenode<T1,T2>* target);
...
template <typename T1,typename T2>
void RBT<T1,T2>::erase(T1 key){    
    treenode<T1,T2>* target=find(key,root);
    if(!target)return;
    else delete_choose(target);
}

template <typename T1,typename T2>
void RBT<T1,T2>::delete_choose(treenode<T1,T2>* target){
    if(target->color==RED){
